...
const source = readFileSync('src/pages/PagesManagerPage.tsx', 'utf8')

assertExcludes(source, 'window.confirm', 'pages manager should use in-app confirmation dialogs')
assertIncludes(source, '<Dialog open={Boolean(confirmDialog)}', 'pages manager confirmation dialog')
